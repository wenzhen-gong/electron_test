...
const Result: React.FC<ResultProps> = (props) => {
    ...
    for (let i = 0; i < state.datafile.length; i++) {
      if (state.datafile[i].sessionId.toString() === sessionId) {
        for (let j = 0; j < state.datafile[i].history.length; j++) {
          if (state.datafile[i].history[j].timestamp.toString() === runId) {
  ...
  const DrawCDF = (data: PercentileStats, label: string) => {
    const keys = Object.keys(data);
    const percentDataY: number[] = [];
    const metricsDataX: number[] = [];
    for (let i = 0; i < keys.length; ++i) {
      if (keys[i][0] === 'p') {
        const percentile = Number(keys[i].substring(1).replace('_', '.'));
        percentDataY.push(percentile);
        metricsDataX.push(data[keys[i] as keyof PercentileStats] as number);
      }
    }
    ...
    return (
      <LineChart
        ...
        yAxis={[{ id: 'Percentile', scaleType: 'log' }]}
        series={[{ yAxisKey: 'Percentile', data: percentDataY, label: label }]}
        leftAxis="Percentile"
        height={400}
      />
    );
  ...
  const [latencyGraphId, setLatencyGraphId] = useState<string | null>(requests[0]);

  let latencyGraphMenuItems: React.ReactNode[] = [];
  ...
  const [requestThroughputGraphId, setRequestThroughputGraphId] = useState<string | null>(null);

  let requestThroughputGraphMenuItems: React.ReactNode[] = [];
  for (let i = 0; i < requests.length; ++i) {
    const requestName = requests[i];
  ...
  const [byteThroughputGraphId, setByteThroughputGraphId] = useState<string | null>(null);

  let byteThroughputGraphMenuItems: React.ReactNode[] = [];
  for (let i = 0; i < requests.length; ++i) {
    const requestName = requests[i];
  ...
  if (latencyGraphId !== null) {
    ...
    graph = DrawCDF(
      resultState.result.requestThroughputStats[requestThroughputGraphId] as PercentileStats,
      requestThroughputGraphId + ' Request Throughput'
    );
  } else {
    graph = DrawCDF(
      resultState.result.byteThroughputStats[byteThroughputGraphId!] as PercentileStats,
      byteThroughputGraphId! + ' Byte Throughput'
    );
  }
  ...
  return (
    <ResultDiv>
      ...
      <div>
        ...
        <label>
          {resultState.concurrentUsers} users on {resultState.numOfWorkers} threads
        </label>
      </div>
      <div>
        <label>Metrics</label>
        <PopupState variant="popover" popupId="demo-popup-menu">
          {(popupState) => (
            <React.Fragment>
              ...
              <Menu {...bindMenu(popupState)}>{latencyGraphMenuItems}</Menu>
            </React.Fragment>
          )}
        </PopupState>
        <PopupState variant="popover" popupId="demo-popup-menu">
          {(popupState) => (
            <React.Fragment>
              ...
              <Menu {...bindMenu(popupState)}>{requestThroughputGraphMenuItems}</Menu>
            </React.Fragment>
          )}
        </PopupState>
        <PopupState variant="popover" popupId="demo-popup-menu">
          {(popupState) => (
            <React.Fragment>
              ...
              <Menu {...bindMenu(popupState)}>{byteThroughputGraphMenuItems}</Menu>
            </React.Fragment>
          )}
        </PopupState>
      </div>
