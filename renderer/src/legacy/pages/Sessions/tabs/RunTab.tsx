...
const RunTab: React.FC = () => {
  ...
  const handleInputChange = (inputName: string, inputValue: string | number): void => {
    ...
    if (inputName === 'URL') {
      config.URL = inputValue as string;
    } else if (inputName === 'httpMethod') {
      config.httpMethod = inputValue as string;
    } else {
      config[inputName] = parseInt(inputValue as string);
    }
  ...
  return (
    ...
    >
      ...
      <TextField
        ...
        onChange={(e) => {
          setConcurrencyNumber(Number(e.target.value));
